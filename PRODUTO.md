...
| Frequência cardíaca | Perfil BLE padrão "Heart Rate" (`react-native-ble-plx`) — conecta a qualquer monitor de peito/pulso que anuncie esse serviço (Polar, Garmin, cintas genéricas), auto-preenche o modal de FC com o bpm ao vivo. **Testado em device físico** em 2026-09-25 (permissão real concedida, scan real iniciado) | Confirmar pareamento/leitura com um monitor BLE físico de verdade (não testado nesta sessão por falta de um à mão) |
...
| Transcrição de voz na nota de fechamento | Grava áudio (`expo-audio`) e transcreve via Gemini (Google AI Studio, tier gratuito) chamado de uma Edge Function no Supabase (a chave da API nunca entra no bundle do app). **Testado ponta a ponta em device físico** em 2026-09-25 (gravou, transcreveu, texto certo apareceu na nota) | Tier gratuito do Gemini responde 503 "high demand" com alguma frequência sob carga — mitigado com uma tentativa extra na function, não eliminado |
